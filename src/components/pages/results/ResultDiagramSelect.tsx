...
const ResultDiagramSelect = (props: Props) => {
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    props.setDiagrams([
      ...selectableDiagrams.map((diagram) =>
        diagram.value == e.target.value
          ? {
              ...diagram,
      ...
      {props.diagrams.map(({ value, label }) => (
        <div key={value} className="flex gap-2">
          ...
          <input
            ...
            id={value}
            className="pl-2"
            aria-label={`Select diagram type: ${label}`}
            onChange={onChange}
            checked={
              selectableDiagrams.find((el) => el.value == value)?.selected
            }
            value={value}
          />
        </div>
